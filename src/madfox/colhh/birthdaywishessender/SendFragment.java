...
import madfox.colhh.birthdaywishessender.SendProgressFragment.SendingResult;
		...
		}else
		{
			if(getResources().getConfiguration().orientation==Configuration.ORIENTATION_PORTRAIT)
			{
				getFragmentManager().beginTransaction()
						.replace(R.id.adz, adf=new AdsFlasher(3000,R.color.l_green, Packages.JOKES_SENDER,
								Packages.QUOTES_SENDER, Packages.XMAS_SENDER))
		...
		switch(view.getId())
		{
			case R.id.send_btn_send:
				if(event.getAction()==MotionEvent.ACTION_DOWN)
				{
					...
					btn_send.setTextColor(Color.rgb(00, 153, 204));
					return true;
				}else if(event.getAction()==MotionEvent.ACTION_UP)
				{
					btn_send.setBackgroundColor(Color.rgb(00, 153, 204));
			...
			case R.id.schedule_btn_send:
				if(event.getAction()==MotionEvent.ACTION_DOWN)
				{
					...
					btn_schedule.setTextColor(Color.rgb(00, 153, 204));
					return true;
				}else if(event.getAction()==MotionEvent.ACTION_UP)
				{
					btn_schedule.setBackgroundColor(Color.rgb(00, 153, 204));
